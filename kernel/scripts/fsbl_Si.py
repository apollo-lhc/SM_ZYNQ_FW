...
def main(out_filename,file_list,i2c_addr_list,serv_offset_list):
    ...
    #build the si programming data and write it to the file so we can reference it later
    #keep track of the preamble and total size for each file
    for iFile in range(0,file_count):
        ...
        preamble_length.append(len(preamble))
        total_length.append(len(preamble)+len(config)+len(postamble))
    ...
    for iFile in range(0,file_count):
        ...
        outfile.write("  localConfigs["+str(iFile)+"].si_enable_AXIAddr = AXI_ADDR_SERV +"+str(serv_offset_list[iFile])+"*sizeof(u32);\n")
        outfile.write("  localConfigs["+str(iFile)+"].si_enable_MASK    = 0x2;\n")
        outfile.write("  localConfigs["+str(iFile)+"].si_oe_AXIAddr     = AXI_ADDR_SERV +"+str(serv_offset_list[iFile])+"*sizeof(u32);\n")
        outfile.write("  localConfigs["+str(iFile)+"].si_oe_MASK        = 0x1;\n")
        outfile.write("  localConfigs["+str(iFile)+"].si_locked_AXIAddr = AXI_ADDR_SERV +"+str(serv_offset_list[iFile])+"*sizeof(u32);\n")
        outfile.write("  localConfigs["+str(iFile)+"].si_locked_MASK    = 0x60;\n")
        outfile.write("  localConfigs["+str(iFile)+"].data              = " + names[iFile] + ";\n")
        outfile.write("  localConfigs["+str(iFile)+"].data_init_size    = " + str(preamble_length[iFile]) + ";\n")
        outfile.write("  localConfigs["+str(iFile)+"].data_size         = " + str(total_length[iFile])+";\n")
        outfile.write("  localConfigs["+str(iFile)+"].data_init_usleep  = 400000;\n")
        outfile.write("  localConfigs["+str(iFile)+"].name              = \"" + names[iFile] + "\";\n")
...
if __name__ == "__main__":
    ...
    if args.files != None and (len(args.files) != len(args.i2caddr) or len(args.files) != len(args.si_serve_offset)):
