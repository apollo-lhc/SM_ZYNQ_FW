...
source ${apollo_root_path}/src/ZynqPS/CPUs/build_CPUs.tcl

proc build_Zynq {bd_name,sm_rev,zynq_type }{
    ...
    #================================================================================
    puts "Building CPU"
    [build_${sm_rev}_${zynq_type}]  
    # sets AXI_MASTER_CLK_FREQ, AXI_MASTER_LOCAL_PORT,AXI_MASTER_C2C_PORT, IRQ_PORT
    ...
    #create an axi FW for the local interconnect
    ...
    connect_bd_intf_net [get_bd_intf_pins ${AXI_MASTER_LOCAL_PORT}] [get_bd_intf_pins ${INT_AXI_FW}/S_AXI]
    ...
    [BUILD_AXI_INTERCONNECT ${AXI_C2C_INTERCONNECT_NAME} ${AXI_MASTER_CLK} ${AXI_MASTER_RSTN} [list ${AXI_MASTER_C2C_PORT}] [list ${AXI_MASTER_CLK}] [list ${AXI_MASTER_RSTN}]]
    set_property CONFIG.STRATEGY {1} [get_bd_cells ${AXI_C2C_INTERCONNECT_NAME}]

    #connect the INT_AXI_FW to the C2C interconnect (so we can reset it if something goes wron on the local interconnect)
    [AXI_CTL_DEV_CONNECT $INT_AXI_FW ${AXI_C2C_INTERCONNECT_NAME} ${AXI_MASTER_CLK} ${AXI_MASTER_RSTN} $AXI_MASTER_CLK_FREQ]    
    #create dt info for the fw
    ...
    connect_bd_net [get_bd_pins ${IRQ_ORR}/dout] [get_bd_pins ${IRQ_PORT}]
    ...
    #create dir for outputted address info
    set AXI_addr_dir=${apollo_root_path}/configs/${build_name}/autogen/AXI_addr/
    file mkdir ${AXI_addr_dir}
    GENERATE_AXI_ADDR_MAP_C    "${AXI_addr_dir}/AXI_slave_addrs.h"
    GENERATE_AXI_ADDR_MAP_VHDL "${AXI_addr_dir}/AXI_slave_pkg.vhd"
    read_vhdl                  "${AXI_addr_dir}/AXI_slave_pkg.vhd"
    ...
    set bd_pdf_dir=${apollo_root_path}/configs/${build_name}/autogen/doc/
    file mkdir ${bd_pdf_dir}
    write_bd_layout -force -format pdf -orientation portrait ${bd_pdf_dir}/${bd_name}.pdf
